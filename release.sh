...
readonly BUNDLE_DIR="cache-action-entrypoint/build/dist/js/productionExecutable"
...
[[ -f "${BUNDLE_DIR}/cache-action-entrypoint.js" ]] || die "no bundle at ${BUNDLE_DIR}"

mkdir -p "${DIST_DIR}"
# the source map is deliberately left out, the runner downloads dist/ on every job
cp "${BUNDLE_DIR}/cache-action-entrypoint.js" "${DIST_DIR}/"
cp "${BUNDLE_DIR}/cache-action-entrypoint.js.LICENSE.txt" "${DIST_DIR}/"

echo "Creating the release commit"
git add "${DIST_DIR}"
# there is no version file to bump, so an unchanged bundle still gets a commit to tag
git commit --allow-empty --message "chore: release ${TAG}"
